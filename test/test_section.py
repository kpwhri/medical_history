...
def test_family_history_section():
    ...
    assert set(results.iter_terms(include_all=True)) == {'pcos', 'family history',
                                                         'family history indicates:', 'daughter'}
...
def test_personal_history_section():
    ...
    assert set(results.iter_terms(include_section=True)) == {'pcos', 'past medical history:'}
